...
let heaviestPokemon = async function(){
    ...
    return heaviestPokemon[0][0]
}

heaviestPokemon().then((heaviestPoke)=>{
    console.log(heaviestPoke.name,"The heviest Pokemon ",
    heaviestPoke.weight,"Kg");
})
...
let findOwners = async function(pokemonName){
    ...
    let trainersArray = [];
    let pokemonIdQuery = `SELECT id FROM pokemon WHERE name = '${pokemonName}'`
...
let mostOwned = async function(){
    ...
    let mostOwnedArray=[];
    let mostOwnedQuery = `SELECT pokemon_id, COUNT(pokemon_id) AS cnt FROM pokemon_trainer GROUP BY pokemon_id ORDER BY COUNT(pokemon_id) desc`
